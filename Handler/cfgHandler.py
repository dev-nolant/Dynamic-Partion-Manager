class ConfigHandler:
    ...
    def load_config(self):
        ...
        # Preprocessing
        # Remove new lines and empty slots
        self.queue = [line.strip() for line in self.config if line.strip()]
        # Comment Handler
        self.queue = [line for line in self.queue if "--" not in line]
    ...
    def getActions(self, path=None):
        ...
        for line in self.queue:
            if line.startswith('>'):
                ...
                action = line[1:].strip()
                # Split the action into key-value pair and add to the actions list
    ...
    def setupDefault(default_partition_allocated_space: str):
        ...
        default_partition_allocated_space = byteManager.ByteDetection(
            default_partition_allocated_space)
        ...
        if not os.path.exists(path):
            ...
            ConfigHandler.addComment(
                path, "This is max partition size per database")
            ConfigHandler.addAction(
                path, "partitions-size", f"{math.ceil(byteManager.Gigabytes(default_partition_allocated_space, reversed=True))}GB")
            ConfigHandler.addComment(
                path, "This is max partitions size per user. This can be modified to accommidate for SaaS")
            ConfigHandler.addAction(path, "indv-partitions-size", "1GB")
            ConfigHandler.addComment(
                path, " -1 to allow unlimited partitions, still limited to partitions size")
            ConfigHandler.addAction(path, "max-partitions", "10")
            ConfigHandler.addComment(
                path, "2GB is default. User will recieve an extra GB for premium")
